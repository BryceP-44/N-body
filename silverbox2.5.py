...
particles=4
size=200 #boundary box size
spawn=100 #spawn box size
seconds=10000 #time
d=.1 #time step
gravityc=1000 #gravitational constant
friction=0 #percent loss in speed during collisions
        ...
        for j in range(particles):
            if j!=i:
                ...
                if r<=(m[j]+m[i]):
                    ...
                    vx[j]=.9*m[i]/m[j]*vx[i]
                    vy[j]=.9*m[i]/m[j]*vy[i]
                    rcx=(1+dd)*(r-m[i]-m[j])*rx/(r+dd)
                    rcy=(1+dd)*(r-m[i]-m[j])*ry/(r+dd)
                    x[i]+=rcx
                    y[i]+=rcy
